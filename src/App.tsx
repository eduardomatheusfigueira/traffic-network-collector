...
export default function App() {
  ...
  // New state for datetime
  const [useCustomTime, setUseCustomTime] = useState(false);
  const [customDate, setCustomDate] = useState('');
  const [customTime, setCustomTime] = useState('');

  // New state for tabs/route
  ...
  const handleApplyKey = () => {
    ...
    setIsKeyApplied(true);
    setTimeout(() => setIsKeyApplied(false), 3000); // Reset success state after 3s
  };
  ...
  const fetchTraffic = async () => {
    ...
    try {
      // Process in batches on the client side to update progress bar
      const batchSize = 50;
      ...
      for (let i = 0; i < segments.length; i += batchSize) {
        ...
        // Update segments state progressively
        setSegments(prev => {
  ...
  const getSegmentColor = (segment: Segment) => {
    if (!segment.duration || !segment.staticDuration) return '#3b82f6'; // blue-500 default
    ...
    if (ratio <= 1.1) return '#22c55e'; // green-500 (fast)
    if (ratio <= 1.5) return '#eab308'; // yellow-500 (moderate)
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input 
                      ...
                      value={tomtomApiKey}
                      onChange={(e) => {
                        setTomtomApiKey(e.target.value);
