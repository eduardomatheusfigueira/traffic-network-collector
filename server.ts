...
app.post('/api/github-upload', async (req, res) => {
  try {
    ...
    const REPO_NAME = 'traffic-network-collector';
    const REPO_DESC = 'Coletor de malha viária e dados de tráfego usando Node, React, OSM e TomTom.';
    ...
    const filesToUpload = [
      'server.ts', 'package.json', 'tsconfig.json', 'tsconfig.app.json', 'tsconfig.node.json',
      'vite.config.ts', 'index.html', 'tailwind.config.js', 'postcss.config.js', 'README.md',
      'src/App.tsx', 'src/index.css', 'src/main.tsx', 'src/vite-env.d.ts',
      'src/components/ui/button.tsx', 'src/components/ui/card.tsx', 'src/components/ui/input.tsx',
      'src/components/ui/progress.tsx', 'src/components/ui/dialog.tsx', 'src/lib/utils.ts'
    ];
    ...
    try {
      ...
      return res.status(401).json({ error: 'Falha na autenticação. Verifique seu token.' });
    }
    ...
    const uploaded = [];
    const currentDir = process.cwd(); // Fallback for ES modules since __dirname is not available
    for (const filePath of filesToUpload) {
      ...
      try {
        ...
        let sha;
        try {
          const fileRes = await api.get(`/repos/${owner}/${REPO_NAME}/contents/${filePath}`);
          sha = fileRes.data.sha;
        } catch (e) { /* Ignore */ }

        await api.put(`/repos/${owner}/${REPO_NAME}/contents/${filePath}`, {
    ...
    res.json({ success: true, url: `https://github.com/${owner}/${REPO_NAME}`, uploaded });
  } catch (error: any) {
    res.status(500).json({ error: error.message });
  }
});
